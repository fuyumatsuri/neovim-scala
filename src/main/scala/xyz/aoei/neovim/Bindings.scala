...
class Neovim(val in: InputStream, val out: OutputStream) extends NeovimBase(in, out) {
  ...
  def setCurrentBuffer(buffer: Buffer): Unit = session.notify("vim_set_current_buffer", buffer)
  ...
  def setCurrentWindow(window: Window): Unit = session.notify("vim_set_current_window", window)
  ...
  def setCurrentTabpage(tabpage: Tabpage): Unit = session.notify("vim_set_current_tabpage", tabpage)
